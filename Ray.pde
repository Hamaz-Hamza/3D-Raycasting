class Ray{
    ...
    Intersection testIntersection(Ray wallRay, color colour){
        ...
        else if (parralel) return new Intersection(rayStart,0, colour);
        else {
            float qSubPCrossWallDir = cross(qSubP, wallRay.dir);
            float rayDist = qSubPCrossWallDir * rxsReciprocal;
            if (rayDist > 0) return new Intersection(rayStart.add(rayDir.mult(rayDist)),rayDist,colour);
            else return null;
        }
    }
